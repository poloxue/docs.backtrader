...
Broker 经纪商模拟器，支持不同的订单类型，检查订单现金需求，跟踪每次 Cerebro 迭代的现金和价值，并维护各数据的当前头寸。

现金在每次迭代中调整，期货等工具的价格变化会导致实际现金增加或减少。
支持的订单类型：
...
由于 Broker 由 Cerebro 实例化，用户通常不需要替换 Broker 实例，参数也不直接受用户控制。

要修改参数，有两种方式：

1. 手动创建带所需参数的 Broker，通过 `cerebro.broker = instance` 设置；
2. 使用 `cerebro.broker.set_xxx()` 方法设置参数。

**注意**，`cerebro.broker` 是 Cerebro 的一个属性，由 `getbroker` 和 `setbroker` 方法支持。
