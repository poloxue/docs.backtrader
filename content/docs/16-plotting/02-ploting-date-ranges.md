...
在 1.9.31.x 版本中，backtrader 增加了部分绘图功能。
...
这对人类来说最直观。也可以使用时间戳的索引：

```python
cerebro.plot(start=75, end=185)
```

以下示例包含简单移动平均线（在数据上绘图）、随机指标（独立绘图）及其交叉信号，通过命令行参数传递给 `cerebro.plot`。
...
Python 的 eval 机制允许在命令行中直接编写 `datetime.date` 并映射为实际意义。输出图表如下：
