...
本节将学习如何使用技术指标，并使用技术指标作为入场和出场条件，将使用的技术指标是简单移动平均线（Simple Moving Average），或称 SMA。

## 交易规则

SMA 是一个非常简单的技术指标，计算一定周期的价格均值。我们先基于 SMA 定义交易规则。

**入场条件：** 当收盘价大于最新的 SMA，则入场买入。  
**出场条件：** 当持有头寸，当收盘价小于 SMA，则出场卖出。

我们之前章节的策略代码大部分可复用，现在要关注就是如何计算技术指标。
...
其中的 `self.params.maperiod` 是定义的策略参数。

如果你成功安装了 talib，backtrader 也集成了 talib 的支持，查看文档 [指标-TALib](/backtrader/docs/08-indicators/04-talib/)。

## 条件判断

现在，我们就可以基于 `self.sma` 判断进出场条件。为了简化代码，这里只考虑 SMA 的判断逻辑，在完整实例中会包含所有情况。
