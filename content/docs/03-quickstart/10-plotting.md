...
通过 `print` 输出每个 bar 的信息不利于阅读，图表视觉效果更直观。backtrader 内置了绘图能力，一行代码即可：

```python
cerebro.plot()
```

需在 `cerebro.run()` 之后调用，且绘图功能依赖 matplotlib。

## 演示

为了展示绘图功能，我们将添加以下指标：

- EMA（指数移动平均线），默认与数据一起绘制
- WMA（加权移动平均线），配置在子图绘制
- StochasticSlow（慢速随机指标），使用默认设置
- MACD，使用默认设置
- ATR，配置为不绘制
- RSI，使用默认设置
- 在 RSI 上叠加 SMA 指标
...
即使指标没有赋值到策略成员变量（如 `self.sma = ...`），它们也会被注册到策略中，成为图表的一部分。
