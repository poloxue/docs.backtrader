...
平台支持通过**时间框架**（如日线、5分钟线）和**压缩级别**（如1天、5分钟）自定义数据，适配不同交易策略。
...
**策略**是交易逻辑的核心，定义了如何根据数据执行买卖操作。Backtrader 中的策略通过继承 `bt.Strategy` 类实现。

### 策略的基本方法

策略类至少需要实现以下两个方法：

1. **`__init__`**：用于定义指标和变量。
2. **`next`**：逐条处理数据，执行交易逻辑。
...
3. **`notify_order`**：订单状态变化时调用。
4. **`notify_trade`**：交易状态变化时调用。
...
- **买入 / 卖出 / 平仓（buy / sell / close）**：
  通过这些方法，策略可以向经纪人发送订单。平台也允许手动创建订单并传递给经纪人，但使用内建方法更简单高效。
...
没有特殊需求时，默认配置会自动处理以下内容：

- 创建默认经纪人
- 操作不收佣金
- 数据源预加载
- 默认执行模式为 `runonce`（批处理模式），这是最快的方式
...
上例中，`period` 从 10 到 45（每次递增 5）进行测试，平台自动运行每个参数组合的回测。
...
通过这些核心组件的整合，**Backtrader** 提供了高度灵活的框架，适用于各种回测和实时交易需求。
